...
The `link:../cheatsheet/VertxOptions.html[VertxOptions]` object has many settings and allows you to configure things like clustering,
...
`link:../cheatsheet/VertxOptions.html[VertxOptions]` object before creating the Vertx object.
...
If you want to deploy a verticle as a worker verticle you do that with `link:../cheatsheet/DeploymentOptions.html#worker[worker]`.
...
This configuration is then available via the `link:groovydoc/io/vertx/groovy/core/Context.html[Context]` object or directly using the
`config` method. The configuration is returned as a JSON object so you can
...
system properties, use `System.getProperty`
(or `System.getProperty`). You can also retrieve the
environment variables using `System.getenv`.
...
options.extraClasspath = ["lib/jars/some-library.jar"]
...
Extra classpath entries can also be provided with `link:../cheatsheet/DeploymentOptions.html#extraClasspath[extraClasspath]` so they
...
cluster managers at run-time by using the Java `ServiceLoader` functionality to locate instances of
`ClusterManager` on the classpath.
...
`link:../cheatsheet/VertxOptions.html#clusterManager[clusterManager]`.
...
class which implements the interface `LoggerFactory`. We provide pre-built implementations for

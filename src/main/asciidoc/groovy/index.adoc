...
You can't do much in Vert.x-land unless you can commune with a `link:../../groovydoc/io/vertx/groovy/core/Vertx.html[Vertx]` object!
...
The `link:../dataobjects.html#VertxOptions[VertxOptions]` object has many settings and allows you to configure things like clustering,
...
`link:../dataobjects.html#VertxOptions[VertxOptions]` object before creating the Vertx object.
...
By default blocking code is executed on the Vert.x blocking code pool, configured with `link:../dataobjects.html#VertxOptions#setWorkerPoolSize(int)[workerPoolSize]`.
...
// 2 minutes
def maxExecuteTime = 120000

def executor = vertx.createSharedWorkerExecutor("my-worker-pool", poolSize, maxExecuteTime)
...
Coordination of multiple asynchronous results can be achieved with Vert.x `link:../../groovydoc/io/vertx/groovy/core/Future.html[futures]`. It
supports concurrent composition (run several async operations in parallel) and sequential composition
(chain async operations).

=== Concurrent composition

`link:../../groovydoc/io/vertx/groovy/core/CompositeFuture.html#all(io.vertx.core.Future,%20io.vertx.core.Future)[CompositeFuture.all]` takes several futures arguments (up to 6) and returns a future that is
...
def httpServerFuture = Future.future()
httpServer.listen(httpServerFuture.completer())

def netServerFuture = Future.future()
...
_waits_ for the first succeeded future. `link:../../groovydoc/io/vertx/groovy/core/CompositeFuture.html#any(io.vertx.core.Future,%20io.vertx.core.Future)[CompositeFuture.any]` takes several futures
...
`link:../../groovydoc/io/vertx/groovy/core/CompositeFuture.html#join(io.vertx.core.Future,%20io.vertx.core.Future)[CompositeFuture.join]` takes several futures arguments (up to 6) and returns a future that is
...
While `all` and `any` are implementing concurrent composition, `link:../../groovydoc/io/vertx/groovy/core/Future.html#compose(io.vertx.core.Handler,%20io.vertx.core.Future)[compose]` can be used
...
* `link:../../groovydoc/io/vertx/groovy/core/Future.html#compose(java.util.function.Function)[compose]`: when the current future completes,
run the given function, that returns a future. When this returned future completes, it completes the composition.
* `link:../../groovydoc/io/vertx/groovy/core/Future.html#compose(io.vertx.core.Handler,%20io.vertx.core.Future)[compose]`: when the current future
...
You can use `link:../../groovydoc/io/vertx/groovy/core/Future.html#completer()[completer]` that completes a future with the operation result or
...
If you want to deploy a verticle as a worker verticle you do that with `link:../dataobjects.html#DeploymentOptions#setWorker(boolean)[worker]`.
...
You can deploy a verticle using one of the `link:../../groovydoc/io/vertx/groovy/core/Vertx.html#deployVerticle(io.vertx.core.Verticle)[deployVerticle]` method, specifying a verticle
...
You can also programmatically register and unregister verticle factories using `link:../../groovydoc/io/vertx/groovy/core/Vertx.html#registerVerticleFactory(io.vertx.core.spi.VerticleFactory)[registerVerticleFactory]`
and `link:../../groovydoc/io/vertx/groovy/core/Vertx.html#unregisterVerticleFactory(io.vertx.core.spi.VerticleFactory)[unregisterVerticleFactory]` if you wish.
...
Deployments can be undeployed with `link:../../groovydoc/io/vertx/groovy/core/Vertx.html#undeploy(java.lang.String)[undeploy]`.
...
`link:../dataobjects.html#DeploymentOptions#setIsolatedClasses(java.util.List)[isolatedClasses]`-  an entry can be a fully qualified
...
Extra classpath entries can also be provided with `link:../dataobjects.html#DeploymentOptions#setExtraClasspath(java.util.List)[extraClasspath]` so if
...
If you are embedding Vert.x and you have finished with it, you can call `link:../../groovydoc/io/vertx/groovy/core/Vertx.html#close()[close]` to close it
down.
...
To retrieve the context, use the `link:../../groovydoc/io/vertx/groovy/core/Vertx.html#getOrCreateContext()[getOrCreateContext]` method:
...
`link:../../groovydoc/io/vertx/groovy/core/Context.html#runOnContext(io.vertx.core.Handler)[runOnContext]`:
...
The context object also let you access verticle configuration using the `link:../../groovydoc/io/vertx/groovy/core/Context.html#config()[config]`
...
To set a timer to fire once you use `link:../../groovydoc/io/vertx/groovy/core/Vertx.html#setTimer(long,%20io.vertx.core.Handler)[setTimer]` method passing in the delay and a handler
...
You can also set a timer to fire periodically by using `link:../../groovydoc/io/vertx/groovy/core/Vertx.html#setPeriodic(long,%20io.vertx.core.Handler)[setPeriodic]`.
...
In this case, you should consider using `link:../../groovydoc/io/vertx/groovy/core/Vertx.html#setTimer(long,%20io.vertx.core.Handler)[setTimer]` instead. Once your treatment has
...
To cancel a periodic timer, call `link:../../groovydoc/io/vertx/groovy/core/Vertx.html#cancelTimer(long)[cancelTimer]` specifying the timer id. For example:
...
Verticle use the Vert.x worker pool for executing blocking actions, i.e `link:../../groovydoc/io/vertx/groovy/core/Context.html#executeBlocking(io.vertx.core.Handler,%20boolean,%20io.vertx.core.Handler)[executeBlocking]` or
worker verticle.
...
For example if you have deployed a verticle which creates a `link:../../groovydoc/io/vertx/groovy/core/net/NetServer.html[NetServer]` which provides
`link:../../groovydoc/io/vertx/groovy/core/net/NetSocket.html[NetSocket]` instances in it's handler, then it's best to always access that socket instance
...
`link:../dataobjects.html#MetricsOptions#setFactory(io.vertx.core.spi.VertxMetricsFactory)[factory]`.
...
`link:../dataobjects.html#VertxOptions#setClusterManager(io.vertx.core.spi.cluster.ClusterManager)[clusterManager]`.
...
DNS servers can be also configured when creating a `link:../../groovydoc/io/vertx/groovy/core/Vertx.html[Vertx]` instance:
...
on Linux, it can be configured to a specific value with `link:../dataobjects.html#AddressResolverOptions#setNdots(int)[ndots]`.

...
If you want to deploy a verticle as a worker verticle you do that with `link:../cheatsheet/DeploymentOptions.html#setWorker(boolean)[setWorker]`.
...
The verticle name is used to look up the specific `link:groovydoc/io/vertx/groovy/core/spi/VerticleFactory.html[VerticleFactory]` that will be used to
...
This configuration is then available via the `link:groovydoc/io/vertx/groovy/core/Context.html[Context]` object.
...
options.extraClasspath = java.util.Arrays.asList("lib/jars/some-library.jar")
...
Extra classpath entries can also be provided with `link:../cheatsheet/DeploymentOptions.html#setExtraClasspath(java.util.List)[setExtraClasspath]` so they
...
`link:groovydoc/io/vertx/groovy/core/spi/metrics/VertxMetrics.html[API Documentation]`.

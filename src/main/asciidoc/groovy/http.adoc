...
To receive unknown frames, you can use the `link:../../groovydoc/io/vertx/groovy/core/http/HttpServerRequest.html#unknownFrameHandler(io.vertx.core.Handler)[unknownFrameHandler]` on the request,
...
To send such frames, you can use the `link:../../groovydoc/io/vertx/groovy/core/http/HttpServerResponse.html#writeFrame(int,%20int,%20io.vertx.core.buffer.Buffer)[writeFrame]` on the response.
Here's an example:
...
`link:../dataobjects.html#HttpClientOptions#setH2cUpgrade(boolean)[h2cUpgrade]` options is set to false: after the
...
The `link:../../groovydoc/io/vertx/groovy/core/http/HttpConnection.html[HttpConnection]` offers the API for dealing with HTTP/2 connection events, lifecycle
and settings.
...
Connection `link:../../groovydoc/io/vertx/groovy/core/http/HttpConnection.html#close()[close]` close is a shutdown with no delay, the `GOAWAY`
frame will still be sent before the connection is closed.

The `link:../../groovydoc/io/vertx/groovy/core/http/HttpConnection.html#closeHandler(io.vertx.core.Handler)[closeHandler]` notifies when connection is closed,
`link:../../groovydoc/io/vertx/groovy/core/http/HttpConnection.html#shutdownHandler((@io.vertx.codegen.annotations.Nullable :: io.vertx.core.Handler))[shutdownHandler]` notifies when all streams have been closed but the
...
The proxy can be configured in the `link:../dataobjects.html#HttpClientOptions[HttpClientOptions]` by setting hostname, port and optionally
...
options.proxyUsername = "username".proxyPassword = "secret"
def client = vertx.createHttpClient(options)

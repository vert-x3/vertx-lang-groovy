...
When using `link:../../groovydoc/io/vertx/groovy/core/eventbus/EventBus.html#send(java.lang.String,%20java.lang.Object)[send]` the event bus attempts to deliver the message to a
`link:../../groovydoc/io/vertx/groovy/core/eventbus/MessageConsumer.html[MessageConsumer]` registered with the event bus.

In some cases it's useful for the sender to know when the consumer has received the message and "processed" it.

To acknowledge that the message has been processed the consumer can reply to the message by calling `link:../../groovydoc/io/vertx/groovy/core/eventbus/Message.html#reply(java.lang.Object)[reply]`.
...
clustered, the port and host, as you would do with `link:../dataobjects.html#VertxOptions#setClustered(boolean)[clustered]`,

...
Be aware that the List will contain the `link:groovydoc/io/vertx/groovy/core/dns/MxRecord.html[null]` sorted by the priority of them, which
means MX records with smaller priority coming first in the List.

The `link:groovydoc/io/vertx/groovy/core/dns/MxRecord.html[null]` allows you to access the priority and the name of the MX record by offer methods for it like:

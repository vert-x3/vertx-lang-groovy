...
Buffers can create by using one of the static `link:groovydoc/io/vertx/groovy/core/buffer/Buffer.html#buffer()[null]` methods.
...
Use `link:groovydoc/io/vertx/groovy/core/buffer/Buffer.html#length()[null]` to obtain the length of the buffer.
The length of a buffer is the index of the byte in the buffer with the largest index + 1.

=== Copying buffers

Use `link:groovydoc/io/vertx/groovy/core/buffer/Buffer.html#copy()[null]` to make a copy of the buffer

=== Slicing buffers

A sliced buffer is a new buffer which backs onto the original buffer, i.e. it does not copy the underlying data.
Use `link:groovydoc/io/vertx/groovy/core/buffer/Buffer.html#slice()[null]` to create a sliced buffers
